...
### 主翻译器 (translator.py)
1. **高性能PDF文本提取**：使用PyMuPDF引擎，比传统方案快30-45倍，有效解决首行缩进导致的段落识别问题
2. **按页数分批翻译**：按指定页数（如每5页）自动分批处理，无需预定义章节
3. **句子完整性处理**：智能检测分批边界，自动扩展到句子结束确保连贯性
4. **自动标题识别**：识别文档中的标题并在翻译中保持Markdown格式
5. **自动编号整合**：所有批次按顺序编号，最终整合为完整文档
6. **增量术语表**：自动维护和更新翻译术语表，确保术语一致性
7. **实时质量监控**：自动检测段落数量差异，标记需要优化的批次
8. **LLM成本跟踪**：实时显示Token使用量和翻译成本
